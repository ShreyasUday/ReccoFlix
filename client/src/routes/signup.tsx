...
import { Mail, Lock, User, Film, ArrowRight, Loader2 } from "lucide-react";
import { useAuth } from "@/lib/auth-context";

export const Route = createFileRoute("/signup")({
  head: () => ({ meta: [{ title: "Create account — ReccoFlix" }] }),
        ...
        <div className="absolute inset-0 flex flex-col justify-between p-10">
          ...
          <div className="space-y-4">
            <h2 className="text-4xl font-extrabold leading-tight">
              Join <span className="text-gradient-primary">2M+ otakus</span><br /> tracking together.
            </h2>
            <p className="max-w-md text-sm text-muted-foreground">
              Build your library, get smart recommendations, never miss a season.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="relative flex min-h-[100dvh] items-center justify-center px-6 py-12 sm:px-12 lg:min-h-0 lg:py-10">
        <div className="w-full max-w-md space-y-6">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-muted-foreground">It's free, forever.</p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3">
            <a href="http://localhost:3000/api/auth/google" className="rounded-xl glass-strong flex items-center justify-center py-3 text-sm font-semibold transition-smooth hover:bg-white/10">
...
function Field({ icon: Icon, label, type = "text", placeholder }: { icon: React.ElementType; label: string; type?: string; placeholder: string }) {
  return (
    <div>
      <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-muted-foreground">{label}</label>
      <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-background/40 px-3 py-2.5 transition-smooth focus-within:border-primary/60 focus-within:shadow-glow">
        <Icon className="h-4 w-4 text-muted-foreground" />
        <input type={type} placeholder={placeholder} className="flex-1 bg-transparent text-sm focus:outline-none" />
      </div>
    </div>
  );
}
